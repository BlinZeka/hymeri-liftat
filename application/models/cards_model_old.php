...
class Cards_Model extends CI_Model {
	...
	public function insertData($arr,$card_no) {
		...
		if (self::checkDuplicateCardNr($card_no) == 1) {
			echo '<div style= "color:red">This card is already registered to <a href = " '.base_url() . 'index.php/cards/index/'.$query[0]['id'].' "> '. $query[0]['first_name'] .' '.$query[0]['first_name'].'</a>.</div>';
			die;
		} else {
...
/*
		...
		$id = $id['id'];
*/
// 		$this->db->query("update cards set client_id = NULL, employer_id = NULL, card_no = $clientID where id = $clientID");
